...
from my_utils import ROIs, find_violation
...
def generate_video(img):
    path_result = os.path.join('output')
    # for i in range(len(img)):
    #     #plt.imshow(img[i], cmap=cm.Greys_r)
    #     plt.savefig(path_result + "/file%02d.png" % i)
        ...
        while self.vdo.grab():
            ...
            if len(outputs) > 0:
                ...
                for bb_xyxy in bbox_xyxy:
                    ...
                    ##
                    (x1, y1) = (bb_xyxy[0], bb_xyxy[1])
                    (x2, y2) = (bb_xyxy[2], bb_xyxy[3])
                    # find the bottom center position and convert it to world coordinate
                    p_c = np.array([[(x1 + x2) / 2], [y2], [1]])
                    transform_cam2world = np.loadtxt('calibration/' + "oxford_town" + '_matrix_cam2world.txt')
            ...
            pts_roi_world, pts_roi_cam = get_roi_pts(dataset="oxford_town", roi_raw=ROIs["oxford_town"],matrix_c2w=transform_cam2world)
            if len(pts_world) > 0:
