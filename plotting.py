...
def plot_frame_one_row(dataset, img_raw, pts_roi_cam, pts_roi_world, pts_w, pairs):
    ...
    fig.suptitle('%s (%s)' % ("YOLO+deepsort", "social distancing"))
    ...
    a.xaxis.label.set_color('red')

    print_str = 'Violation is detected: '
    for pair in pairs:
        print_str += str((int(pts_w[pair[0]][0]),int(pts_w[pair[0]][1])))
        print_str += str((int(pts_w[pair[1]][0]), int(pts_w[pair[1]][1])))
    a.set_xlabel(print_str)
